...
BEGIN
    --THIS is the exception block, you may execute code here
...
/*
    ...
    execution block only                         CAN be used in queries OR executon block
*/
...
--EXEC
BEGIN
    insert_food_null_id('jaw breaker', 'one quarter');
END;
/
...
---VIEW
/*in sql, a view is a virtual table based on the result of a sql statement.
A veiw contains rows and columns just like a real table...BUT...it is NOT a real
table. The fields in a view are fields from one or more REAL tables.
You can add SQL functions, WHERE clauses, JOIN clauses, etc to a view and
...
/* a cursor is a temporary work area created in the system memory when a SQL
    statement has executed. A cursor contains information on a select statement
    and the rows of data accessed by it. (basically, it points to a result set
    AND remembers which line numbers it already read so far).
